...
def read_kds_tile(
    ...
    instance: bool = False,
    remap: bool = False,
    #extra argument added for KDS
    rgb=False,
    max_intensity=600) -> Data:
    """Read a KDS tile saved as .laz.
    ...
    :param remap: bool
        Whether semantic labels should be mapped from their KDS ID
        to their train ID.
    """

    """Read a KDS file saved as LAZ."""
    data = Data()
    key = 'testing'
    ...
    if semantic:
        ...
        if remap:
            ...
            if max_class >= len(ID2TRAINID):
                extended = np.full(max_class + 1, fill_value=KDS_NUM_CLASSES, dtype=np.int64)
...
class MiniKDS(KDS):
    """A mini version of KDS with only a few windows for
    experimentation.
    """
    _NUM_MINI = 2
